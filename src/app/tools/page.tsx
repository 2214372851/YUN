...
import React, { useState, useEffect, useMemo } from 'react'; // 引入 useState, useEffect, useMemo
import Link from 'next/link';
import { getAllTools, Tool } from '@/data/tools';
import { Button } from "@/components/ui/button"; // 引入 Button 用于筛选器
import { cn } from "@/lib/utils"; // 引入 cn 用于条件样式
...
function ToolCard({ tool }: ToolCardProps) {
  ...
  const cardContent = (
    <div className="block rounded-lg border border-white/5 bg-card hover:bg-[hsl(var(--linear-gray))/0.1] group-hover:border-white/20 transition-colors overflow-hidden p-1 h-full">
      {/* 图片区域已移除 */}
      <div className="p-4">
